...
command = 'python3 {}'.format(test) + '--dataroot {}'.format(opt.data_dir) +\
          ' --phase test --loadSize 512  --netG_A global --netG_B global' +\
          ' --ngf 32 --niter_fix_global 0 --n_blocks_global 8 --n_downsample_global 2' +\
...
base_dir = './result/{}/test_{}/'.format(opt.name, opt.epoch)
result_dir = base_dir + 'images/'
save_dir = base_dir
command = 'python3 datasets/extract.py' + ' --dir {} --save_dir {}'.format(result_dir, save_dir)
...
command = 'python3 evaluation/precalc_stats_inception.py'
command1 = command + ' --dir {}'.format(fake_A_dir)
command2 = command + ' --dir {}'.format(fake_B_dir)

print("----------------------------------------------------------------------")
print("[FID Score] calculate inception score")
...
if not os.path.exists(fid_result_dir+'test_A.npz'):
    ...
    command = 'python3 precalc_stats_fid.py --data_dir {} --out_dir {}'\
...
command3 = 'python3 precalc_stats_fid.py --data_dir={} --out_dir={}'\
    .format(fake_B_dir, fid_result_dir+'{}_fake_B.npz'.format(opt.name))

os.system(command3)

command4 = 'python3 precalc_stats_fid.py --data_dir={} --out_dir={}'\
...
command5 = 'python3 fid.py {}test_B.npz {}{}_fake_B.npz  --gpu 0'.format(fid_result_dir, fid_result_dir, opt.name)
os.system(command5)

print("[FID SCORE] Test A and Fake A")
command6 = 'python3 fid.py {}}test_A.npz {}{}_fake_A.npz  --gpu 0'.format(fid_result_dir, fid_result_dir,opt.name)
os.system(command6)

print("[FID SCORE] Train B and Train B")
command7 = 'python3 fid.py {}train_B.npz {}{}_fake_B.npz  --gpu 0'.format(fid_result_dir, fid_result_dir, opt.name)
os.system(command7)

print("[FID SCORE] Train A and Train A")
command7 = 'python3 fid.py {}}train_A.npz {}{}_fake_A.npz  --gpu 0'.format(fid_result_dir, fid_result_dir,opt.name)
os.system(command7)
